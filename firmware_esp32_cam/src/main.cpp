...
bool initCamera() {
  ...
  config.xclk_freq_hz = 20000000; // Tăng lên 20MHz để FPS mượt hơn
  config.pixel_format = PIXFORMAT_JPEG;

  // Test ổn định với QVGA trước
  ...
  // TỐI ƯU CÀI ĐẶT SENSOR ĐỂ HÌNH ẢNH RÕ NÉT HƠN
  sensor_t *s = esp_camera_sensor_get();
  if (s) {
    s->set_brightness(s, 2);    // Tăng độ sáng lên tối đa (+2)
    ...
    s->set_aec2(s, 1);          // Bật AEC2 để bù sáng tốt hơn
    s->set_ae_level(s, 2);      // Tăng mức phơi sáng lên tối đa
    s->set_gain_ctrl(s, 1);     // Bật AGC
    s->set_agc_gain(s, 30);     // Tăng Gain
  }

  Serial.println("✅ Camera khởi tạo và cấu hình sensor thành công!");
  return true;
}
...
void handleStream() {
  ...
  while (client.connected()) {
    // NHÚNG MQTT LOOP VÀ RECONNECT VÀO ĐÂY ĐỂ ĐẢM BẢO LUÔN NHẬN ĐƯỢC LỆNH FLASH
    ...
    if (!client.connected())
      break;
    delay(1); // Để 1ms để đạt FPS cao nhất
  }
...
void setup() {
  ...
  // Cấu hình chân LED Flash là ngõ ra để cấp điện ổn định
  pinMode(FLASH_LED_PIN, OUTPUT);

  // Test Flash LED khi khởi động (Bật 0.5s rồi tắt)
  digitalWrite(FLASH_LED_PIN, HIGH);
  delay(500);
  digitalWrite(FLASH_LED_PIN, LOW);
